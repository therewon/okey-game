...
export function OkeyTile({
  ...
  const className = `okey-tile ${compact ? "tile-compact" : ""} ${selected ? "tile-selected" : ""}`;
  const label = tile.isFakeJoker ? "Saxta joker" : `${tile.color} ${tile.number}${tile.isOkey ? ", Okey" : ""}`;
  const content = tile.isFakeJoker ? (
  ...
  if (!onClick && !draggable) {
    return <span className={`${className} tile-static`} role="img" aria-label={label}>{content}</span>;
  }
